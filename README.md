...
ChaChaAgent 是一个可扩展的通用 AI Agent 框架，提供从模型调用、上下文管理到工具执行的全链路编排。采用**微内核控制平面**设计，核心编排层与能力插件层解耦，支持 CLI（Textual TUI）与 Web（FastAPI + React）双前端，并可接入 MCP 协议工具、Code-RAG 知识引擎、子 Agent 等高级能力。

## 架构全景图

```
表现层 (双前端)
  ├─ CLI (Textual TUI)          终端主界面：消息滚动、PTY终端、审批弹窗
...
记忆与上下文子系统
  ...
  ├─ MemoryManager              读写MEMORY.md，Auto Dream清洗
...
```bash
cp chachaConfig.example.toml chachaConfig.toml
```
...
├── interface/cli/       Textual TUI 终端界面
...
| [钩子开发](docs/hooks.md) | 钩子类型、责任链顺序、自定义钩子示例 |
| [记忆系统](docs/memory.md) | MEMORY.md 格式、自动剪枝、调优建议 |
