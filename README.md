...
- ✅ Think-Act-Observe 主循环 — 流式工具调用，含自动压缩、审批、钩子
...
核心编排层 (微内核)
  ├─ Orchestrator       ✅  Think-Act-Observe 主循环，会话记忆自动保存
  ├─ ChatEngine         ✅  消息历史 + 上下文组装 + 自动压缩 + 检查点
  ├─ Dispatcher         ✅  LLM↔工具桥接，工具结果缓存，动态断路器
...
│   ├── chat_engine.py       对话引擎 (历史+压缩+检查点)
│   ├── dispatcher.py        LLM↔工具桥接调度器
