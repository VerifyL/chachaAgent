...
- 创建 MemoryManager（`.chacha/` 目录结构按需自动创建）
- 会话检查点保存在 `.chacha/checkpoints/`

1. **宪法文件**: 若 `~/.chacha/CHACHA.md` 不存在，从模板自动复制
2. **配置文件**: 若 `~/.chacha/config.toml` 不存在，自动生成默认模板
3. **记忆目录**: MemoryManager 自动创建 `.chacha/projects/{hash}/memory/` 目录结构
...
| 文件 | 位置 | 用途 |
|------|------|------|
| debug.jsonl | `.chacha/logs/debug.jsonl` | 研发调试，5 级过滤（DEBUG/INFO/WARNING/ERROR/CRITICAL） |
| audit.jsonl | `.chacha/logs/audit.jsonl` | 安全审计，记录工具调用、成本、记忆变更、权限审批 |

**查看日志**：

```bash
# 实时跟踪调试日志
tail -f .chacha/logs/debug.jsonl | python -m json.tool

# 过滤特定级别
grep '"level":"ERROR"' .chacha/logs/debug.jsonl

# 审计日志分析
cat .chacha/logs/audit.jsonl | jq '.category'
```
...
检查点全量保存 `ConversationState`（events + metadata + loop_state）到 `.chacha/checkpoints/{session_id}/{checkpoint_id}.json`。
