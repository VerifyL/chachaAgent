...
| 权限 | 工具 | 原因 |
|------|------|------|
| **FREE** | read, grep, glob, memory, cache_read, approval_control | 只读/查询，无副作用 |
| **ASK_FIRST** | write, edit, bash, task | 修改文件系统 / 执行命令 |
| **APPROVE_ONCE** | （会话级一次性授权，第二次放行） | 任务级授权 |
...
```python
decision = engine.evaluate_tool("write", "write config.json", "s1")
...
# 5 分钟内再次调用
decision2 = engine.evaluate_tool("write", "write config.json", "s1")
...
仅 `edit`、`write`、`bash` 三个写入类工具会触发检测。`path` / `diff` / `command` 参数被扫描，命中后直接返回 blocked。
...
# 写文件 → ASK_FIRST → 每次审批
d = engine.evaluate_tool("write", "write config.json", "s1")
assert d.needs_approval
# 用户确认后
engine.record_approval(d.cache_key, approved=True)

# 危险命令 → 直接拦截
d = engine.evaluate_tool("bash", "rm -rf /", "s1")
...
d = engine.evaluate_tool("bash", "DROP TABLE users", "s1", risk_factors=factors)
