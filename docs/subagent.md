...
`core/subagent/` 参考 Claude Code sub-agent 设计，实现 LLM 自主委托子Agent 执行独立任务。
...
| 类型 | 用途 | 工具 | max_iter | skip_claude_md |
|------|------|------|----------|----------------|
...
# 主工具列表
parent_tools = ToolExecutor(tools=[read_file, grep, edit_file])
...
- `skip_claude_md`：explore 不加载 CHACHA.md（避免污染上下文）
- 硬超时 300s（可配置）
