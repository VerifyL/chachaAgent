...
def test_count_block(counter):
    block = ContextBlock(
        ...
        content="Hello world", zone="dynamic", priority=3,
    )
...
def test_count_blocks(counter):
    blocks = [
        ContextBlock(source=BlockSource.SYSTEM_PROMPT, role="system",
                     content="system prompt", zone="protected", priority=0),
        ContextBlock(source=BlockSource.HISTORY, role="user",
                     content="user msg", zone="dynamic", priority=3),
    ]
