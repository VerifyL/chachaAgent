...
async def test_text_only_replay_with_telemetry():
    ...
    client = MockClient(
        [
            ...
            DoneChunk(finish_reason="stop", usage={"input": 50, "output": 10, "model": "gpt-4"}),
        ]
    )
    invoker = LLMInvoker(model_client=client, telemetry=t)
    resp = await invoker.invoke([{"role": "user", "content": "Say hello"}])
...
async def test_tool_call_replay():
    ...
    client = MockClient(
        [
            ...
            ToolCallStartChunk(tool_index=0, tool_id="call-1", tool_name="read_file"),
            ToolCallEndChunk(tool_index=0),
            DoneChunk(finish_reason="tool_calls", usage={"input": 100, "output": 20}),
        ]
    )
    invoker = LLMInvoker(model_client=client)
    resp = await invoker.invoke([{"role": "user", "content": "Read main.py"}])
