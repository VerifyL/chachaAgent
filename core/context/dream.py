...
class DreamPipeline:
    ...
    def _gather(self, memory_manager) -> str:
        ...
        # sessions/{sid}/ 下的每日文件（按天收集）
        days = memory_manager.list_days(limit=30)
        for day in days:
            text = memory_manager.read_day(day)
            if text.strip():
                parts.append(f"## session/{day}.md\n{text}")
...
_GLOBAL_DREAM_ROUNDS = 50  # 跨 session 累计 50 轮触发
_GLOBAL_DREAM_HOURS = 72   # 缺 72 小时未更新触发

GLOBAL_DREAM_PROMPT = """You are a long-term learning assistant. Your task: merge ALL session summaries into a cumulative project memory.

## Input:
1. ALL session MEMORY.md files (each summarizes one session's knowledge)
2. CURRENT CHACHA_MEMORY.md (existing permanent memory)

## Rules:
1. Review old CHACHA_MEMORY.md entries (each has [id:xxx]). For each: [KEEP]/[UPDATE]/[DELETE]
2. Extract new patterns from session MEMORY.md files:
   - Long-term user preferences and habits
   - Critical/irreversible project decisions  
   - Hard-won lessons and reusable insights
   - Error patterns likely to recur
   - Project milestones
3. Assign unique IDs with prefixes: pref-xxx, dec-xxx, learn-xxx, err-xxx, prog-xxx
4. No entry limit — keep ALL truly persistent information
5. Output ONLY the merged CHACHA_MEMORY.md content (no commentary)"""


class GlobalDream:
    """跨 session 长期记忆（单例，阈值由 SessionService 配置）。

    收集所有 sessions/*/MEMORY.md → 提炼 CHACHA_MEMORY.md。
    """

    _instance = None

    def __init__(self, llm_invoker=None, base_dir=None,
                 dream_rounds=_GLOBAL_DREAM_ROUNDS, dream_hours=_GLOBAL_DREAM_HOURS):
        self._llm = llm_invoker
    ...
    def should_run(self) -> bool:
        if self._last_run is None:
            return self._total_rounds >= self._dream_rounds
        return (self._total_rounds >= self._dream_rounds or
                time.time() - self._last_run > self._dream_hours * 3600)

    async def run(self) -> str:
        from core.context.memory_manager import MemoryManager
        if not self._llm:
            return "GlobalDream: 未配置 LLM"
        logger.info("GlobalDream 开始跨 session 学习...")
        mgr = MemoryManager(project_id="default", base_dir=self._base_dir)
        old_permanent = mgr.read_permanent_memory()
        session_mds = mgr.list_all_session_dirs()
        parts = [f"## {md.parent.name}\n{md.read_text().strip()}"
                 for md in session_mds if md.read_text().strip()]
        if not parts and not old_permanent:
            return "GlobalDream: 无数据"
        collected = "\n\n".join(parts)
        user = f"## OLD CHACHA_MEMORY.md\n{old_permanent}\n\n## ALL\n{collected}" if old_permanent else collected
        resp = await self._llm.invoke(
            messages=[{"role": "system", "content": GLOBAL_DREAM_PROMPT},
                       {"role": "user", "content": user}],
            session_id="global-dream",
        )
        mgr.write_permanent_memory(resp.text.strip())
        self._last_run = time.time()
        self._total_rounds = 0
