...
class ContextBlock(BaseModel):
    ...
    compression_level: CompressionLevel = Field(
        ...
        description="当前压缩层级，从 NONE 渐进到 CONSOLIDATED"
    )
    original_token_count: Optional[int] = Field(
        None, ge=0,
        description="压缩前 token 数（FROZEN/TRIMMED/SUMMARIZED 时记录，用于审计）"
    )
    ...
    frozen_total_lines: Optional[int] = Field(
        None, ge=0,
        description="冻结前原始总行数"
    )
...
class ContextAssemblyMeta(BaseModel):
    ...
    trigger: str = Field(
        "normal",
        description="组装触发类型: normal | compression | first_turn | recovery"
    )
    ...
    budget_per_task: Optional[int] = Field(
        None, ge=1,
        description="任务级 token 预算（含多次调用），为空则不限"
    )

    # ---------- 压缩决策 ----------
    utilization_ratio: float = Field(
        0.0, ge=0.0, le=2.0,
        description="total / budget_per_request，>1 表示已超限"
    )
...
"""
    ...
    trigger_reason: TriggerReason = Field(
        TriggerReason.NONE,
        description="当前压缩触发原因"
    )

    # ---------- 推理预算（思考预算分离追踪）----------
    reasoning_budget_tokens: int = Field(
        0, ge=0,
        description="思考 token 配额（0 表示无限制/不使用）"
    )
    reasoning_tokens_used: int = Field(
        0, ge=0,
        description="当前会话累计思考 token 消耗"
    )

    # ---------- 来源分布分析 ----------
    blocks_by_source: Dict[str, int] = Field(
        default_factory=dict,
        description="各来源的 token 分布，格式: {source: token_count}"
    )
...
class AssembledContext(BaseModel):
    ...
    meta: ContextAssemblyMeta = Field(
        default_factory=ContextAssemblyMeta,
        description="组装元信息"
    )
    blocks: List[ContextBlock] = Field(
        default_factory=list,
        description="按 priority 排序的上下文片段列表"
    )

    needs_compression: bool = Field(
        False,
        description="utilization_ratio > trigger_ratio 时为 True"
    )
    recommended_level: CompressionLevel = Field(
        CompressionLevel.NONE,
        description="基于 compression_pressure 推荐的压缩层级"
    )
    ...
    def get_statistics(self) -> str:
        ...
        if self.meta.reasoning_tokens_used:
            stat_parts.append(
                f"思考: {self.meta.reasoning_tokens_used} / {self.meta.reasoning_budget_tokens}"
            )
        if self.meta.blocks_by_source:
            distribution = " | ".join(
                f"{src}: {tokens}" for src, tokens in self.meta.blocks_by_source.items()
            )
